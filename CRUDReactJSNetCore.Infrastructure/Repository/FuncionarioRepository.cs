...
namespace CRUDReactJSNetCore.Infrastructure.Repository
{
    public class FuncionarioRepository : RepositoryBase<Funcionario>, IFuncionarioRepository
    {
        ...
        public Task<List<Funcionario>> ListFuncionarios(int pageIndex, int pageCount, bool addDesativados, string filter, bool addRelationships = true)
        {
            var query = _dbContext.Funcionarios.AsQueryable();

            if (addDesativados == false)
        ...
        public async Task<IEnumerable<Funcionario>> ListGestores(long level)
        {
            var query = _dbContext.Funcionarios
                        .Where(x => x.Active == true && x.Cargo.Level < level);
